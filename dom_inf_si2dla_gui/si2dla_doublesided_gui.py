...
def execute_algorithm(D_list):
    ...
    result = si2dla(D_list, R_list, S_list)
    if result != False:
        (T_f, T_g) = result
    else:
        rev_D_list = []
        ...
        (T_f, T_g) = si2dla(rev_D_list, R_list, S_list, True)
    ...
    print(T_f.Q[0])
    graph_f.edge("T_f", "T_f", label=tr_label)
    ...
    for tr in T_g.E:
        if (tr[0], tr[3]) not in state_mappings:
            if str(tr[2]) is not tr[2]: # bandaid
                tr2 = tr[2][0]
            else:
                tr2 = tr[2]
            state_mappings[(tr[0], tr[3])] = [tr[1] + ': ' + tr2]
        else:
